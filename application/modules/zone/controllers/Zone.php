...
class Zone extends Generic{
	...
	$list = $this->Mdl_zone->get_datatabless("zone");
//	echo '<pre>'; print_r($list); exit;
    ...
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $this->Mdl_zone->count_alls(),
      "recordsFiltered" => $this->Mdl_zone->count_filtereds(),
      "data" => $data,
    );
	...
	function addZoneAction(){
		...
		if($this->form_validation->run() == FALSE){
			...
			$adminId = $admin['admin_id'];
				...
				$insert = $this->Mdl_zone->insert("zones",$data); 
				echo 1; exit;			
		}
	   } else {
	...
	function editZone($id){
		...
		if(!empty($id)){
			$services = $this->Mdl_zone->getByValue("zones","id",$id);
			if($services == "No Data"){
	...
	function editZoneAction(){
		...
		if($this->form_validation->run() == FALSE){
			echo validation_errors();
				...
				$update = $this->Mdl_zone->update2("zones","id",$content['id'],$data);
				echo 1; exit;
		}
	}
